<template>
  <div v-on-click-outside="hidePlaceInput" v-on:click="showPlaceInput()">
    <div>
      <span v-if="!showAddressSelector && !keepOpen">{{addressAlias ? addressAlias + ',' : ''}}{{value | formatAddress | formatEmpty}}</span>
    </div>
    <div v-if="showAddressSelector || keepOpen" >
      ...
      ></GmapAutocomplete>
      <input type="number" min="0" max="100" v-if="value" :value="value.floor" @input="(e) => updatedModel({floor: e.target.value})" placeholder="קומה">
      <input type="text" v-if="value" :value="value.enterance" @input="(e) => updatedModel({enterance: e.target.value })" placeholder="כניסה">
     </div>
  </div>
</template>
