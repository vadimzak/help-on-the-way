drop function if exists help."update_ticket_due_date_for_volunteer_by_id"(id bigint, due_date date, due_time time with time zone);
drop function if exists help."getTicketById"(ticket_id bigint);
drop function if exists help."getTicketByIdForVolunteer"(ticket_id bigint);
...
drop function if exists help."getCurrentVolunteerAssignedTickets"();
drop function if exists help_private."getCurrentUserIdGracefully"();

drop view if exists help.ticket_for_volunteer;

ALTER TABLE help.ticket
ALTER due_time TYPE time USING due_time AT TIME ZONE 'UTC+3';

create or replace view help.ticket_for_volunteer as
SELECT
    ticket.*,
    (array(select volunteer_id from help.ticket_assigned_volunteers tbl where tbl.ticket_id = ticket.id)) as assigned_volunteers,
...
CREATE OR REPLACE FUNCTION help.update_ticket_due_date_for_volunteer_by_id(id bigint, due_date date, due_time time)
RETURNS help.ticket_for_volunteer AS $$
    ...
    var data = {"due_date": due_date, "due_time": due_time, status: scheduledStatus}

    plv8.execute("select help.update_ticket_for_volunteer_by_id($1, $2);", id, data);
$$ LANGUAGE plv8 security definer;
GRANT EXECUTE ON FUNCTION help.update_ticket_due_date_for_volunteer_by_id(id bigint, due_date date, due_time time) TO "VOLUNTEER";
