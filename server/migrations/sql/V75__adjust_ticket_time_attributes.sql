...
ALTER due_date type date using due_date::date,
ADD column due_time timetz,
ADD COLUMN fixed_due_time boolean,
ADD column expiry_date timestamptz,
add column modified_at timestamptz,
ALTER modified_at SET DEFAULT now();
...
CREATE OR REPLACE FUNCTION help.update_ticket_due_date_for_volunteer_by_id(id bigint, due_date date, due_time timetz)
...
$$ LANGUAGE plv8 security definer;
GRANT EXECUTE ON FUNCTION help.update_ticket_due_date_for_volunteer_by_id(id bigint, due_date date, due_time timetz) TO "VOLUNTEER";
